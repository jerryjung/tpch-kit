...
from
  o_customer c join
    ( select n_name, l_extendedprice, l_discount, s_nationkey, o_custkey from o_orders o join
      ( select n_name, l_extendedprice, l_discount, l_orderkey, s_nationkey from o_lineitem l join
        ( select n_name, s_suppkey, s_nationkey from o_supplier s join
          ( select n_name, n_nationkey 
            from o_nation n join o_region r 
