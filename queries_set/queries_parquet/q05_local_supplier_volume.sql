...
from
  p_customer c join
    ( select n_name, l_extendedprice, l_discount, s_nationkey, o_custkey from p_orders o join
      ( select n_name, l_extendedprice, l_discount, l_orderkey, s_nationkey from p_lineitem l join
        ( select n_name, s_suppkey, s_nationkey from p_supplier s join
          ( select n_name, n_nationkey 
            from p_nation n join p_region r 
